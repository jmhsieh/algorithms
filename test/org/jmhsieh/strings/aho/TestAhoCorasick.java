...
import org.jmhsieh.strings.aho.AhoCorasick;
import org.jmhsieh.strings.aho.SearchResult;
import org.jmhsieh.strings.aho.State;
...
public class TestAhoCorasick extends TestCase {
  ...
  private AhoCorasick<byte[]> tree;

  @Override
  public void setUp() {
    this.tree = new AhoCorasick<byte[]>();
  }
  ...
  public void testStartSearchWithSingleResult() {
    ...
    tree.prepare();
    SearchResult<byte[]> result = tree
        .startSearch("washington cut the apple tree".getBytes());
  ...
  public void testStartSearchWithAdjacentResults() {
    ...
    tree.prepare();
    SearchResult<byte[]> firstResult = tree.startSearch("johnjane".getBytes());
    SearchResult<byte[]> secondResult = tree.continueSearch(firstResult);
  ...
  public void testIteratorInterface() {
    ...
    tree.prepare();
    Iterator<SearchResult<String>> iter = tree
        .search("one moon ago".getBytes());

    assertTrue(iter.hasNext());
    SearchResult<String> r = iter.next();
    assertEquals(Arrays.asList(new String[] { "on" }), r.getOutputs());
    assertEquals(2, r.getLastIndex());

    assertTrue(iter.hasNext());
    r = iter.next();
    assertEquals(Arrays.asList(new String[] { "one", "ne" }), r.getOutputs());
    assertEquals(3, r.getLastIndex());

    assertTrue(iter.hasNext());
    r = iter.next();
    assertEquals(Arrays.asList(new String[] { "moo" }), r.getOutputs());
  ...
  public void largerTextExample() {
    ...
    for (Iterator<SearchResult<String>> iter = tree.search(text.getBytes()); iter
        .hasNext();) {
      SearchResult<String> result = iter.next();
      termsThatHit.addAll(result.getOutputs());
    }
    assertEquals(Arrays.asList(new String[] { "cytochrome", "GA3",
